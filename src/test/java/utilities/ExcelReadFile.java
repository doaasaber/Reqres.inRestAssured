...
public class ExcelReadFile {
    ...
    public String getCellData(String sheetName, int row, int columnNum){
        sheet = workbook.getSheet(sheetName);
        getAllHeaderColumnsValues();
        String data = null;
        try {

                data = sheet.getRow(row).getCell(columnNum).getStringCellValue();

        }

        catch (Exception e) {
            double data2 = sheet.getRow(row).getCell(columnNum).getNumericCellValue();
            int data3=(int)data2;
